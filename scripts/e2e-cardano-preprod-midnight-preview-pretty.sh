#!/usr/bin/env bash
#
# NuAuth E2E — Cardano Preprod (Blockfrost) + Midnight Preview
#
#   ./scripts/e2e-cardano-preprod-midnight-preview-pretty.sh
#
# Prereqs:
#   • .env: BLOCKFROST_*, WALLET_* / CREATOR_*, BUYER_*, ABE_MASTER_KEY_HEX, BIP39_MNEMONIC
#   • Preprod ADA for stamp + list + license
#   • Midnight Preview wallet funded with tNIGHT AND DUST registration complete
#
set -euo pipefail
...
OUT_JSON="${NUAUTH_E2E_OUT:-/tmp/nuauth-e2e-midnight-preview-summary.json}"
PARTIAL_JSON="${OUT_JSON%.json}-cardano-partial.json"
API="${API_BASE:-http://127.0.0.1:${API_PORT:-8788}}"
export NUAUTH_DATA_DIR="${NUAUTH_E2E_DATA_DIR:-/tmp/nuauth-e2e-preview-$$}"
...
check_midnight_preview_indexer() {
  local url="https://indexer.preview.midnight.network/api/v3/graphql"
  local code
  code="$(curl -sS -m 8 -o /tmp/nuauth-gq.json -w '%{http_code}' \
...
export MIDNIGHT_DEPLOY_NETWORK=preview

if [[ "${NUAUTH_E2E_CLEAR:-}" == "1" ]]; then clear 2>/dev/null || true; fi
banner "NuAuth E2E — Cardano Preprod + Midnight Preview"
kv "Midnight" "MIDNIGHT_DEPLOY_NETWORK=${MIDNIGHT_DEPLOY_NETWORK} (public Preview)"
kv "Cardano" "${CARDANO_BACKEND} / ${CARDANO_NETWORK:-Preprod}"
kv "API" "$API"
kv "Data dir" "$NUAUTH_DATA_DIR"
rule

step_do "Preflight: Midnight Preview indexer (api/v3/graphql)"
if check_midnight_preview_indexer; then
  step_ok "Preview indexer reachable"
else
  warn "Preview indexer preflight failed (network/DNS). Continuing; Midnight CLI may still succeed."
fi
...
PAYLOAD="$(printf '%s' "nuauth-e2e-preview $(date -Iseconds)" | base64 | tr -d '\n')"

step_do "POST /api/creator/register"
REG_JSON="$(curl -fsS -X POST "${API}/api/creator/register" \
  -H 'Content-Type: application/json' \
  -d "{\"filename\":\"e2e-preview.txt\",\"contentBase64\":\"${PAYLOAD}\"}")"
...
step_do "npm run midnight:run-all (Midnight Preview — deploy + ZK circuits)"
echo ""
MID_LOG="$(mktemp)"
...
jq -n \
  ...
  '{
    ...
    explorers: {
      ...
      midnightPreviewIndexer: "https://indexer.preview.midnight.network/"
    }
  }' > "$OUT_JSON"
...
echo -e "\n${BOLD}${GREEN}Midnight — Preview${NC}"
rule
printf "  %b%-22s%b\n" "$DIM" "deployTxHash" "$NC"
mono_line "$DEPLOY_TX"
